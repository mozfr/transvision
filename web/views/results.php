...
if ($locale == $locale2)
	...
	{
	$search_results = $results->getTMXResults3(array_keys($locale1_strings), $tmx_source, $tmx_target,$tmx_target2);
	echo '<h2><span class="searchedTerm">' . $initial_search . '</span> in ' . $sourceLocale . ':</h2>';
	echo ShowResults::resultsTable3($search_results, $initial_search, $sourceLocale, $locale,  $locale2, $check);

	$search_results = Utils::results3(array_keys($locale2_strings), $tmx_source, $tmx_target,$tmx_target2);
	echo '<h2><span class="searchedTerm">' . $initial_search . '</span> in ' . $locale . ':</h2>';
	echo ShowResults::resultsTable3($search_results, $initial_search, $sourceLocale, $locale,$locale2 ,$check);

	$search_results = Utils::results3(array_keys($locale3_strings), $tmx_source, $tmx_target,$tmx_target2);
